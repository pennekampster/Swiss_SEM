...
# Data source
Data stem from [*Grassland ecosystem recovery after soil disturbance depends on nutrient supply rate*](https://onlinelibrary.wiley.com/doi/abs/10.1111/ele.13591).
...
- functional.group: Species functional group (C3 Grass, C4 Grass, Forb, Legume, Sedge) (char)	C3=C3 Grass, C4=C4 Grass, F=Forb, L=Legume, S=Sedge
...
```{r}
rm(list = ls())
setwd("~/Swiss_SEM/data_preparation/CedarCreek_Seabloom/seabloom-2020-ele-dryad-data/")
```
...
```{r}
seabloom <- lapply(list.files(pattern = "csv")[-4], read.table, sep = ",", 
                   header = TRUE)
...
First, rename the `mass.above` according to their group.
```{r}
## Total biomass
...
```{r}
comb <- merge(data, functional, by = c("field", "exp", "year", "plot", "disk",
                                       "ntrt", "nadd", "other.add", "dur"))


comb <- merge(comb, life, by = c("field", "exp", "year", "plot", "disk"))
