...
\centering
![](images/correlation.jpg){width=50%}
...
\centering ![](images/LMind2_full_mediation.pdf){width=30%}
\centering ![](images/LMind2_partial_mediation.pdf){width=30%}
\centering ![](images/LMind2_no_mediation.pdf){width=30%}
...
| intercept | `~ 1` | estimate mean for `y` without `x`s | `y ~ 1` |
| labelling | `*` | name coefficients | `y ~ b1*x1 + b2*x2` |
| defining | `:=` | define quantity | `TotalEffect := b1*b3 + b2` |
...
- Perform mediation analysis:
  - Is the effect of disturbance is mediated via its effect on richness and eveness, rather than directly on biomass.
