...
-   Participants understand the capabilities and limits of SEMs to draw inferences from data
...
-   Everything else being equal, variation in X leading to variation in Y (=X causes Y).
-   Causal inference methods when experiments not feasible.
:::
...
1)  Understand which drivers influence a system ('driver-focused SEM')
2)  Understand and explain a particular property of a system ('response-focused')
3)  Understand causal pathways ('mediation-focused')
4)  Evaluate available theory ('theory-focused')
...
![](images/CedarCreek_exp_design.png)
...
![](images/Seabloom_PathDiagram.pdf)
...
digraph  {
  ...
  # a 'graph' statement
  graph [overlap = false, fontsize = 20, layout = dot, splines = curved,
  ...
  nutrients; evenness; richness; biomass; disturbance;
...
digraph {
  ...
  # 'edge' statements
  X -> Y1 [label = <&beta;<SUB>X,Y1</SUB>> minlen=3 ];
