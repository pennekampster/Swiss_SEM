...
::: {.incremental}
- Indicator variables "emergent" from the latent variable.   
...
- Regression approaches assume that explanatory variables are measured without error.  
...
y <- 2*x 
y2 <- y + rnorm(50, sd=4)
ggplot(data=NULL) + geom_point(aes(x=x,y=y2)) +  stat_smooth( aes(x=x,y=y2), method="lm", se=F, colour="blue") + stat_smooth(aes(x=x,y=y), method="lm", colour="red", se=F) + scale_colour_manual(values = c("red", "blue")) 
```


::: {.incremental}
- Red = true slope, blue slope estimated with OLS.
...
- Latent variable with multiple indicator variables.
- Will lead to better slope estimate and higher R^2^.
:::
...
- Arrows pointing from the indicators to the composite (formative or causal indicators).
...
- Flow of causation: 
  ...
  - *Composite*: flow of causation is reversed and the indicators are independent entities.   
...
- Moderation modelled with interaction term.
...
```r
group.equal=c("intercepts", "means", "regressions", "residuals", "residual.covariances")
```
