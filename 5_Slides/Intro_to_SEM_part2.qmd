...
- Latent variable can have from 1 to many indicators:
  - 1 indicator: 
    - assumes measured variable is perfect representation of the latent. 
    - allows to make a conceptual distinction between a concept and the observed measure.

. . .

  - 2 indicators:
    - requires imposing constraints to identify the parameters. 
    - e.g., equal loadings of latent on indicators, or fixing error variances.
...
2)  Structural model
    -   Estimates relationships between multiple latent variables and between latent and other variables.
...
-   Multiple regression assumes no error in X.\
...
- Body size is multidimensional.
  ...
  - Identifiability: 
    ...
    - Because of this, use standardized coefficients for interpretation.
...
- Technically, composites = prediction from linear combinations of variables.
:::
...
- Can be modelled by adding a squared term of nutrients to regression
...
-   Effect of one variable may depend on another variable.   
    +   E.g., nutrient effect on plant growth may depend on past disturbance.
-   The interaction coefficient describes positive/negative deviation from additivity.
:::
...
::: incremental
- Composite that includes both main effects and their interaction.
- Best approach when only a few interactions are considered and main effects are continuous.
:::
...
- Even more control by naming and fixing parameters
  ...
  - Fixing coefficients to 0 allows causal graphs to vary among groups.
::: 
...
-   Construct a latent variable called diversity using richness and evenness as indicators:
    -   Use confirmatory factor analysis assuming the same loadings of indicators.
...
- Compare starting model with this one. Which one is better supported?
