...
-   The latent variable itself is unobserved.
-   Instead, its influence is captured by indicator variable(s).\
-   Latents capture the common variance or overlapping information for a set of measures. 
-   Direction of causality flows from latent variable to the observed variable(s).
...
- Latent variable can have 1 to many indicators:
  ...
  - 2 indicators:
    - need to make additional assumptions to identify the model. 
...
-   This results in better slope estimate and higher R^2^ when regressing Y on latent rather than indicators.
:::
...
- Body size is multidimensional.
  ...
  - Identifiability: 
    - Loading of first indicator set to 1 by lavaan (sets scale of latent).
