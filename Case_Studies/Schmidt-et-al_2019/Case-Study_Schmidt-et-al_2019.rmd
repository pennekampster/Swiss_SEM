...
Unsurprisingly, this initial model performs really badly. We look at the model summary, identify those path coefficients that are non-significant (_p_ > 0.10), and trim the model accordingly:

```{r results = 'hide'}
# Trim non-significant paths (p > 0.10)
mod.algae.2 <- '
  # Latent
  Algae =~ BC_2.RelAbun + BC_3.RelAbun + BC_4.RelAbun
  ...
  TP_MED_42DAY ~ UrbanTotal2011 + SandContent + AgricultureTotal2011 + r50_ForestTotal2011 + RBS
  Tmax42.C.OBSYN ~ SandContent + BasinArea + PctCanopy
  Algae ~ PctCanopy + NH3_MED_42DAY + TN_MED_42DAY + TP_MED_42DAY + Tmax42.C.OBSYN
...
This model performs a lot better already. But are we missing any important links? Modification indices measure the distance between fitted and implied covariance matrices and suggest additional paths that could improve model fit. Here, we are interested in modification indices > 4:

```{r}
m.i <- modindices(fit.mod.algae.2, standardized = FALSE); print(m.i[m.i$mi > 4.0,])
```

Most suggestions do not make sense biologically. Could there exist a path between "Agriculture Land Use" and "Algae"? We add this path and compare the two models using AIC~C~. For this, we (install and) load the package `AICcmodavg` and use the function `aictab()`:

```{r echo = FALSE, results = 'hide'}
# Add potential missing link
mod.algae.3 <- '
  # Latent
  Algae =~ BC_2.RelAbun + BC_3.RelAbun + BC_4.RelAbun
  ...
  TP_MED_42DAY ~ UrbanTotal2011 + SandContent + AgricultureTotal2011 + r50_ForestTotal2011 + RBS
  Tmax42.C.OBSYN ~ SandContent + BasinArea + PctCanopy
  Algae ~ AgricultureTotal2011 + PctCanopy + NH3_MED_42DAY + TN_MED_42DAY + TP_MED_42DAY + Tmax42.C.OBSYN
...
aictab(list(fit.mod.algae.2, fit.mod.algae.3), c("Model 2", "Model 3"))
```

This link does not improve our model, and so we ignore it. Let us inspect whether any further path coefficients are deemed non-significant in our second algal model. We trim `Algae ~ PctCanopy` and `Algae ~ TN_MED_42DAY` and compare the models again:
...
mod.algae.4 <- '
  ...
  TP_MED_42DAY ~ UrbanTotal2011 + SandContent + AgricultureTotal2011 + r50_ForestTotal2011 + RBS
  Tmax42.C.OBSYN ~ SandContent + BasinArea + PctCanopy
  Algae ~ NH3_MED_42DAY + TP_MED_42DAY + Tmax42.C.OBSYN
...
```{r}
# Compare algal models
aictab(list(fit.mod.algae.2, fit.mod.algae.4), c("Model 2", "Model 4"))
```

Indeed, removing these two links further improved our model. Again, let us use a robust fit approach using Bollen-Stine bootstrap (_n_ = 1000):
...
# Bollen-Stine bootstrap
fit.mod.algae.4.bs <- sem(mod.algae.4, data = dat3, test = "bollen.stine", se = "boot", bootstrap = 1000, fixed.x = FALSE)
head(standardizedSolution(fit.mod.algae.4.bs)) # we use head() to only display the first few rows
```

And report the covariance matrix in the supplementary material:
