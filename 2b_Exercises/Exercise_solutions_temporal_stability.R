#####################################################  
### Using temporal stability data to practice SEM ###
### just using temporal stability does not have   ##
## advantages over using biomass data.            ##
## Model also mean and sd of mass above          ##
#####################################################
...
seabloom <- seabloom %>% group_by(exp, field, plot, disk, yr.plowed, ntrt, nadd, other.add) %>% summarise(
                                                                                       mass.above.mean = mean(mass.above),
                                                                                       mass.above.sd = sd(mass.above),
                                                                                       # calculate inverse cofficient of variation of mass.above as measure of temporal stability
                                                                                       temp.stab = mass.above.mean/mass.above.sd,
                                                                                       rich = mean(rich),
                                                                                       even = mean(even),
                                                                                       ens.pie = mean(ens.pie)) 
...
vars_to_check <- c("nadd", "disk", "rich", "even", "ens.pie", "mass.above.mean", "mass.above.sd", "temp.stab")
...
lm.dir <- lm(temp.stab ~ nadd  + rich + even, data = seabloom)
summary(lm.dir)
par(mfrow=c(2,2))
plot(lm.dir)
par(mfrow=c(1,1))
...
simple <-
"temp.stab ~ nadd + rich + even + disk + mass.above.mean + mass.above.sd 
rich ~ nadd
even ~ nadd"
...
simple.up2 <-
"temp.stab ~ nadd + rich + even + disk + mass.above.mean + mass.above.sd 
mass.above.mean ~ rich + even
rich ~ nadd
even ~ nadd
even ~~rich "

fit.simple2 <- sem(simple.up2, data = seabloom, estimator = "MLM")
summary(fit.simple2, fit.measures = TRUE)


seabloom$mass.above.mean <- seabloom$mass.above.mean / 100
seabloom$mass.above.sd <- seabloom$mass.above.sd / 100

fit.simple2 <- sem(simple.up2, data = seabloom, estimator = "MLM")
summary(fit.simple2, fit.measures = TRUE)

# modification indices
modindices(fit.simple2, minimum.value = 3.84)

simple.up2 <-
"temp.stab ~ nadd + rich + even + disk + mass.above.mean + mass.above.sd 
mass.above.mean ~ rich + even + nadd + disk
mass.above.sd ~ rich + even + nadd + disk
rich ~ nadd
even ~ nadd
even ~~rich 
mass.above.mean ~~   mass.above.sd"
...
derived <-
"temp.stab ~ b1 * nadd + b2 * rich + b3 * even + disk
rich ~ b4 * nadd 
even ~ b5 * nadd 
...
prune <-
"temp.stab ~ rich  +  mass.above.mean + mass.above.sd 
mass.above.mean ~ rich + even + nadd + disk
mass.above.sd ~ rich + even + nadd + disk
rich ~ nadd 
even ~ nadd 
...
simple <- "temp.stab ~ rich  +  mass.above.mean + mass.above.sd 
mass.above.mean ~ rich + even + nadd + disk
mass.above.sd ~ rich + even + nadd + disk
rich ~ nadd 
even ~ nadd 
...
temp.stab ~ mass.above.mean + mass.above.sd + diversity
mass.above.mean ~ diversity + nadd + disk
mass.above.sd ~ diversity + nadd + disk
mass.above.mean ~~  mass.above.sd"
...
int <- "temp.stab ~ disk + nadd + diskxnadd"
