...
Here we clearly have a problem with model mis-specification. The variable ens.pie is a linear combination of evenness and species richness. This causes the model to fail. Therefore we abandon the ens.pie variable for a moment and focus on a two indicator latent variable.

Remember that two indicator latent variables are not identifiable because we only have one piece of information two estimate two path coefficients.
...
diversity =~ lambda*rich_std + lambda*even.rev_std
