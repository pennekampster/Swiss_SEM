...
seabloom$rich <- seabloom$rich / 10
...
plot(seabloom$ens.pie, seabloom$rich)
```
...
```{r}
seabloom$ens.pie.rev <- 1 - seabloom$ens.pie
seabloom$ens.pie2 <- seabloom$rich * seabloom$even
...
plot(seabloom$ens.pie.rev, seabloom$rich)
plot(seabloom$ens.pie.rev, seabloom$even.rev)
...
seabloom$even_std <- (mean(seabloom$even)-seabloom$even) / sd(seabloom$even)

diversity <- 'div =~ lambda*rich_std + lambda*even_std'

diversity <- 'div =~ 1*even + lambda*rich_std'


# setting the same lambda for all indicators to achieve model identification
# diversity <- 'div =~ lambda*rich + lambda*ens.pie'
...
lv <-"
# Latent variable definition
precip =~ lambda*precip.mm + lambda*precip.gs
precip~~0*precip
"
...
lv <-"
# Latent variable definition
precip =~ precip.mm + precip.gs

mass.above ~ nadd + disk + precip
rich ~ nadd + precip + disk
even ~ nadd + precip + disk"
...
summary(fit.int.mg.constrain, fit.measures = TRUE, standardized = TRUE, rsq = TRUE)
```

Let's visualize again the coefficients to see the effect:

```{r, results=F}
fit_tab <- (summary(fit.int.mg.constrain, fit.measures = TRUE, standardized = TRUE, rsq = TRUE))$PE
fit_tab$term <- paste0(fit_tab$lhs, " ", fit_tab$op, " ", fit_tab$rhs)
fit_tab$group_chr <- ifelse(fit_tab$group == 1, "undist", 
                            ifelse(fit_tab$group == 2, "dist", NA))
