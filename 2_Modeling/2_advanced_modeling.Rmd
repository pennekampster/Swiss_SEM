...
Composites are often symbolized by hexangular shapes or--the same the latent variable--by ellipses.
...
As evenness is negatively correlated with richness, we take its inverse ():
```{r}
...
fit.diversity <- sem(diversity, data = seabloom, estimator = "MLM")
...
```{r}
modindices(fit.int.full, minimum.value = 3.84)
```
