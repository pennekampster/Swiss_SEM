...
first the fit between the latent variables (these models are referred to as and their indicators [confirmatory factor analysis (CFA)](https://en.wikipedia.org/wiki/Confirmatory_factor_analysis)), second the full model [@grace21a].
...
subset(modindices(fit.diversity), mi > 3.0 & op == "~~")
```
...
**The variance of `env.rev` is negative (`r round(lavInspect(fit.diversity, "est")$theta[2, 2], digits = 2)`) and if `even` is used instead of `even.rev`, the variance of `div` becomes negative...`**
...
### CFA with pre-calculated coefficients
Works neither...

```{r}
# div <- "mass.above ~ rich + even.rev"
# fit.div <- sem(div, seabloom, estimator = "MLM")
# summary(fit.div)
# 
# beta_rich <- lavInspect(fit.div, "est")$beta[1, 2]
# beta_even <- lavInspect(fit.div, "est")$beta[1, 3]
# 
# div <- "div =~ beta_rich * rich + beta_even * even.rev"
# 
# fit.diversity <- cfa(div, data = seabloom, estimator = "MLM")
# # Request modification indices – error correlations only
# subset(modindices(fit.diversity), mi > 3.0 & op == "~~")
```
...
## Composite variable
### Landuse composite
To model the collective effect of the treatment, let's create a "landuse" composite variable that accounts for disturbance and nutrient addition.
...
## Multigroup fitting to incorporate interactions
...
summary(fit.int.mg, fit.measures = TRUE, standardized = TRUE, rsq = TRUE)
```
...
```{r}
fit_tab <- (summary(fit.int.mg, fit.measures = TRUE, standardized = TRUE, rsq = TRUE))$PE
...
Furthermore, we can test equality constraints by labeling parameters. To do so, we provide namees for the path coefficients that we want to be equal for both groups (here "b1" and "b2").

```{r}
int.mg.constrain <-
"mass.above ~ rich + even + precip.mm
rich ~ c('b1', 'b1') * nadd + precip.mm
even ~ c('b2', 'b2') *  nadd + precip.mm

rich ~~ even"

fit.int.mg <- sem(int.mg.constrain, group = "disk", data = seabloom)
summary(fit.int.mg, fit.measures = TRUE, standardized = TRUE, rsq = TRUE)
```
...
```{r}
seabloom$lat[seabloom$field == "A"] <- 45.4232 + rnorm(length(seabloom$lat[seabloom$field == "A"]), mean=0, .001)
seabloom$lon[seabloom$field == "A"] <- -93.197 + rnorm(length(seabloom$lat[seabloom$field == "A"]), mean=0, .001)

seabloom$lat[seabloom$field == "B"] <- 45.4258 + rnorm(length(seabloom$lat[seabloom$field == "B"]), mean=0, .001)
seabloom$lon[seabloom$field == "B"] <- -93.2102 + rnorm(length(seabloom$lat[seabloom$field == "B"]), mean=0, .001)

seabloom$lat[seabloom$field == "C"] <- 45.3969 + rnorm(length(seabloom$lat[seabloom$field == "C"]), mean=0, .001)
seabloom$lon[seabloom$field == "C"] <- -93.1933 + + rnorm(length(seabloom$lat[seabloom$field == "C"]), mean=0, .001)
```
...
library(ggplot2)
qplot(lat, lon, data=seabloom, color=mass.above) +
  theme_bw(base_size=18) + 
  scale_color_gradient("AGB", low="lightgreen", high="darkgreen")

# residuals are key for the analysis
fitRes <- as.data.frame(residuals_lavaan(fit.simple2))

seabloom$fitted.value <- fitted.values(fit.simple2, "casewise")
seabloom$resid <- seabloom$mass.above - seabloom$fitted.value


#raw visualization of NDVI residuals
ggplot(data=seabloom, aes(lat, lon)) + geom_point(aes(colour=fitRes$mass.above))
