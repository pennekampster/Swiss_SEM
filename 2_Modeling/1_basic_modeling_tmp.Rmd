...
# library(tidyverse)
 seabloom$year <- seabloom$yr.plowed <- NULL
 seabloom <- seabloom %>% group_by(exp, field, plot, disk, ntrt) %>% summarize_at(c("nadd","precip.mm", "precip.gs", "mass.above", "rich", "even", "ens.pie"), mean, na.rm = TRUE)
...
```{r}
ggpairs(seabloom[, c("nadd","precip.mm", "precip.gs", "mass.above", "rich", "even", "ens.pie")], upper=NULL, lower = list(continuous = wrap("points", alpha = 0.05)))  + theme_bw() + theme(panel.grid=element_blank())
```
...
```{r}
ggpairs(seabloom[, c("nadd","precip.mm", "precip.gs", "mass.above", "rich", "even", "ens.pie")], lower = list(continuous = wrap("points", alpha = 0.05)))  + theme_bw() + theme(panel.grid=element_blank())
```
...
mvn(data = seabloom[, c("mass.above", "rich", "even")], mvnTest = "hz", univariatePlot = "qqplot")
```
...
mvn(data = seabloom[, c("log.mass.above", "log.even", "log.rich")], mvnTest = "hz", univariatePlot = "qqplot")
...
boxplot(seabloom[, c(6, 10, 12:15)], las = 2)
```
...
Our robust CFI of `r round(fitmeasures(fit.simple, "cfi.robust"), digits = 2)` is fairly below this threshold. In comparison to the $p$-value, the CFI has the advantage to be independent of sample size.
...
# Model pruning
Now, let's delete all statistically non-significant paths from the saturated model to obtain the most parsimonous model. There is only a single non-significant path, that is, the path between disturbance and evenness. 
...
When comparing the CFI and AIC, we see that the pruned model is slightly superior to the saturated model. The AIC is the lowest (although dAIC < 2) and the number of model parameters is lower and hence this model is the most parsimonous.
...
Since it was removed from [CRAN](https://cran.r-project.org/) beginning of 2021, it needs to be downloaded and installed from the archive.
