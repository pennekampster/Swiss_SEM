...
These correlations have a huge influence on the coefficients between the predictors $x_{1\ldots i}$ and the response $y$.
...
| latent variable | `<~` | set formative indicators | `Comp1 <~ 1*x1 + x2 + x3` |
...
- The $p$-value, which represents the probability of the data given our model (or no significant discrepancy between model and data) [@grace21], should be larger than 0.05.
- The [comparative fit index](https://en.wikipedia.org/wiki/Confirmatory_factor_analysis#Comparative_fit_index) (CFI) ranges between zero and one, whereas a value $> .95$ is considered as a good fit indicator [@hu99]. Our robust CFI of `r round(fitmeasures(fit.simple)[17], digits = 2)` is fairly below this threshold.
...
Now, the model has a decent fit with a ratio of test statistic and degrees of freedom much smaller than two (i.e. `r round(fitmeasures(fit.simple.up)[3] / fitmeasures(fit.simple.up)[4], digits = 2)`) and a $p$-value of `r round(fitmeasures(fit.simple.up)[8], digits = 2)`.
...
Using analysis of covariances allows for estimation of both unstandardized (raw) and standardized parameters [@grace21].
**While the analysis is based on covariances for estimating unstandardized parameters, it is based on correlations for estimating standardized parameters.**
...
Significance testing of standardized estimates violates statistical principles as significance tests are based on unstandardized (raw) parameters.
Thus, standardized effects are used for interpretation only [@grace21].
...
## Model comparison
To evaluate whether the simple or the updated model perform better, we can calculate the [Akaike information criterion](https://en.wikipedia.org/wiki/Akaike_information_criterion) (AIC) and the compute a significance test based on the $\chi^2$ test statistic with the `anova` function.
...
**??? Why is the $\chi^2$ difference not equal to the difference between the two values when using the MLM estimator?**
...
## Summary output
Now, let's have a closer look on the model via `summary` requesting the standardized estimates:
...
Following @grace21, the meaning of the summary output is:
