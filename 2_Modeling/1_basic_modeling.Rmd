...
### Random terms
#### With piecewiseSEM
Let's do now the very same with the package [`piecewiseSEM`](https://cran.r-project.org/web/packages/piecewiseSEM/vignettes/piecewiseSEM.html). It has the advantage that we can include some random structure into the model.
...
#### With `lavaan.survey`
However, also the add-on package [`lavaan.survey`](https://cran.r-project.org/web/packages/lavaan.survey/index.html) allows analysing stratified, clustered and weighted data. Like this, `lavaan` objects can be processed further with random terms:
first, we initialize the design, then, post-process the `lavaan` object and compute the adjusted results.
...
design <- svydesign(ids = ~ field, nest = TRUE, #probs = ~ 1, 
                    data = seabloom)

fit.simple.up.strat <- lavaan.survey(lavaan.fit = fit.simple.up,
                                     survey.design = design)
summary(fit.simple.up.strat, standardized = TRUE)
```
