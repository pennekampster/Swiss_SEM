...
```{r}
pairs(seabloom.agg[, -c(1, 3:4)], lower.panel = NULL)
```
...
```{r}
lm.rich <- lm(rich ~ nadd + precip.mm, data = seabloom.agg)
summary(lm.rich)

lm.even <- lm(even ~ nadd + precip.mm, data = seabloom.agg)
summary(lm.even)
```
...
```{r}
round(cor(seabloom.agg[, c(2:3, 5:10)]), digits = 2)
```
...
mvn(data = seabloom.agg[, c(5, 6, 8)], mvnTest = "hz", univariatePlot = "qqplot")
```
...
First, let's define a function that plots the histogram and the density curve:
```{r}
histWithDensity <- function(variable){
  hist(variable, prob = TRUE, main = "")
...
Then, we can apply this function to our three endogenous variables:
```{r}
par(mfrow = c(1, 3))
histWithDensity(seabloom.agg$mass.above)
histWithDensity(seabloom.agg$rich)
histWithDensity(seabloom.agg$even)
```
...
seabloom.agg$log.mass.above <- log1p(seabloom.agg$mass.above)
seabloom.agg$log.even <- log1p(seabloom.agg$even)
seabloom.agg$log.rich <- log1p(seabloom.agg$rich)


mvn(data = seabloom.agg[, c(11:13)], mvnTest = "hz", univariatePlot = "qqplot")

par(mfrow = c(1, 3))
histWithDensity(seabloom.agg$log.mass.above)
histWithDensity(seabloom.agg$log.rich)
histWithDensity(seabloom.agg$log.even)
```
...
<!-- library(MASS) -->
<!-- ## Richness -->
<!-- Box <- boxcox(seabloom.agg$rich ~ 1, lambda = seq(-6, 6, 0.1)) -->
<!-- Cox <- data.frame(Box$x, Box$y) -->
<!-- Cox2 <- Cox[with(Cox, order(-Cox$Box.y)), ] -->
<!-- Cox2[1, ] -->
<!-- lambda <- Cox2[1, "Box.x"] -->
<!-- seabloom.agg$bc.rich <- (seabloom.agg$rich ^ lambda - 1) / lambda -->

<!-- histWithDensity(seabloom.agg$rich) -->
<!-- histWithDensity(seabloom.agg$bc.rich) -->

<!-- ## Evennness -->
<!-- Box <- boxcox(seabloom.agg$even ~ 1, lambda = seq(-6, 6, 0.1)) -->
<!-- Cox <- data.frame(Box$x, Box$y) -->
<!-- Cox2 <- Cox[with(Cox, order(-Cox$Box.y)), ] -->
<!-- Cox2[1, ] -->
<!-- lambda <- Cox2[1, "Box.x"] -->
<!-- seabloom.agg$bc.even <- (seabloom.agg$even ^ lambda - 1) / lambda -->

<!-- histWithDensity(seabloom.agg$even) -->
<!-- histWithDensity(seabloom.agg$bc.even) -->

<!-- ## Biomass -->
<!-- Box <- boxcox(seabloom.agg$mass.above ~ 1, lambda = seq(-6, 6, 0.1)) -->
<!-- Cox <- data.frame(Box$x, Box$y) -->
<!-- Cox2 <- Cox[with(Cox, order(-Cox$Box.y)), ] -->
<!-- Cox2[1, ] -->
<!-- lambda <- Cox2[1, "Box.x"] -->
<!-- seabloom.agg$bc.mass.above <- (seabloom.agg$mass.above ^ lambda - 1) / lambda -->

<!-- histWithDensity(seabloom.agg$mass.above) -->
<!-- histWithDensity(seabloom.agg$bc.mass.above) -->

<!-- mvn(data = seabloom.agg[, c(5:6, 8, 11:13)], mvnTest = "hz", -->
...
```{r}
fit.simple <- sem(simple, data = seabloom.agg, estimator = "MLM")
```
...
```{r}
seabloom.agg$mass.above <- seabloom.agg$mass.above / 100
seabloom.agg$precip.mm <- seabloom.agg$precip.mm / 100

boxplot(seabloom.agg[, c(2, 5:6, 8:9)])
```

Then, run `sem` again with the rescaled `biomass` and `precip.mm` variable:
...
Our robust CFI of `r round(fitmeasures(fit.simple, "cfi.robust"), digits = 2)` is fairly below this threshold.
...
Including this relation into the model would improve its fit by a change in the $\chi^2$ by `r round(modindices(fit.simple, minimum.value = 3)[1, 4], digits = 3)`.
...
Now, the model has a decent fit with a ratio of test statistic and degrees of freedom smaller than two (i.e. `r round(fitmeasures(fit.simple.up, "chisq") / fitmeasures(fit.simple.up, "df"), digits = 2)`) and a $p$-value of `r round(fitmeasures(fit.simple.up, "pvalue"), digits = 2)`.
Further, also the robust CFI is now `r round(fitmeasures(fit.simple.up, "cfi.robust"), digits = 2)`.
...
fit.satur <- sem(satur, data = seabloom.agg, estimator = "MLM")
