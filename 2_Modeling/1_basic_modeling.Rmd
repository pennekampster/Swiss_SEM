...
par(mfrow = c(2, 2))
hist(seabloom$rich)
qqnorm(seabloom$rich)
qqline(seabloom$rich)

hist(sqrt(seabloom$rich))
qqnorm(sqrt(seabloom$rich))
qqline(sqrt(seabloom$rich))
```
...
Our CFI of `r round(fitmeasures(fit.simple)[13], digits = 2)` is fairly below this threshold.
...
Now, the model has a decent fit with a ratio of test statistic and degrees of freedom much smaller than two (i.e. `r round(fitmeasures(fit.simple.up)[3] / fitmeasures(fit.simple.up)[4], digits = 2)`) and a $p$-value of `r round(fitmeasures(fit.simple.up)[5], digits = 2)`.
Further, also the CFI is now `r round(fitmeasures(fit.simple.up)[9], digits = 2)`.
